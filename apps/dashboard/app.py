...
class PPODirectionalEnv(gym.Env):
    def __init__(self, df, feature_cols):
        ...
        self.observation_space = spaces.Box(low=-10.0, high=10.0, shape=(len(feature_cols),), dtype=np.float32)
        self.current_step = 0
        self.position = 0
        self.reset()
...
class TradingEnv(gym.Env):
    def __init__(self, df):
        ...
        self.feature_cols = feature_cols
        self.action_space = spaces.Discrete(3)
        self.observation_space = spaces.Box(low=-10.0, high=10.0, shape=(len(feature_cols) + 2,), dtype=np.float32)
        self.current_step = 0
        self.position = 0
        self.reset()
    ...
    def step(self, action):
        ...
        obs = self._get_obs() if not done else np.zeros(len(self.feature_cols) + 2, dtype=np.float32)
        return obs, reward, done, False, {}
...
if raw_df is not None:
    ...
    period_sel = st.sidebar.selectbox("Analysis Split", ["Full Backtest", "XGB Train (IS)", "PPO Train (IS)", "OOS Test (OOS)"])
    ...
    timeline_df = pd.DataFrame([
        dict(Split="XGB In-Sample Training (60%)", Start=0, End=split_1, Phase="XGBoost In-Sample"),
        dict(Split="PPO In-Sample Policy Training (20%)", Start=split_1, End=split_2, Phase="PPO Policy Training"),
        dict(Split="Out-Of-Sample Holdout Testing (20%)", Start=split_2, End=len(raw_df), Phase="Holdout Testing")
    ])

    # Precompute state visitation heatmap data globally to avoid NameErrors across tabs
    df["prob_bin"] = pd.cut(df["xgb_prob"], bins=5, labels=["Very Low", "Low", "Neutral", "High", "Very High"])
    df["pos_label"] = [{ -1: "Short (-1)", 0: "Flat (0)", 1: "Long (+1)" }[int(pos)] for pos in df["aegis_pos"]]
    heat_data = df.groupby(["prob_bin", "pos_label"], observed=False).size().unstack(fill_value=0)

    # ==============================================================================
    # 🏥 HEADER LAYOUT
    # ==============================================================================
    st.markdown("<div class='quant-header'>🛡️ PROJECT AEGIS // ADVANCED ANALYTICS</div>", unsafe_allow_html=True)
    st.markdown(f"<div class='quant-subheader'>Quantitative Research Workspace • {period_sel.upper()}</div>", unsafe_allow_html=True)
    ...
    with tab_overview:
        ...
        k1.metric("PORTFOLIO VALUE", f"${df['sim_aegis_equity'].iloc[-1]:,.2f}", f"{(df['sim_aegis_equity'].iloc[-1]/init_cap - 1)*100:.2f}%")
        ...
        fig_eq = go.Figure()
        fig_eq.add_trace(go.Scatter(x=df.index, y=df["sim_aegis_equity"], name="AEGIS Hybrid Policy", line=dict(color="#22C55E", width=2.5)))
        fig_eq.add_trace(go.Scatter(x=df.index, y=df["sim_bh_equity"], name="Buy & Hold (Benchmark)", line=dict(color="#94A3B8", width=1.5, dash="dash")))
        fig_eq.add_trace(go.Scatter(x=df.index, y=df["sim_xgb_equity"], name="XGBoost Signal Only", line=dict(color="#3B82F6", width=1.2, dash="dot")))
        fig_eq.add_trace(go.Scatter(x=df.index, y=df["sim_ppo_equity"], name="PPO Execution Only", line=dict(color="#8B5CF6", width=1.2, dash="dot")))
        ...
        with col_stats:
            fig_dd = go.Figure()
            fig_dd.add_trace(go.Scatter(x=df.index, y=df["aegis_dd"]*100, name="AEGIS Drawdown", fill="tozeroy", fillcolor="rgba(239, 68, 68, 0.05)", line=dict(color="#EF4444", width=1.5)))
            fig_dd.add_trace(go.Scatter(x=df.index, y=df["bh_dd"]*100, name="B&H Drawdown", line=dict(color="#94A3B8", width=1, dash="dash")))
        ...
        with col_heat:
            ...
            try:
                monthly_pnl = temp_dt["sim_aegis_pnl"].resample("M").apply(lambda r: np.prod(1 + r) - 1)
                ...
                pivot_df = pivot_df.reindex(columns=[m for m in months_order if m in pivot_df.columns]).fillna(0.0)
    ...
    with tab_cog:
        ...
        with col_meta:
            st.markdown("<div style='background-color:#162033; padding: 1.5rem; border: 1px solid #273449; border-radius: 4px;'>", unsafe_allow_html=True)
        ...
        with col_diag:
            if selected_node == "1. Market Data":
                # Price profile
                fig_d = px.line(df, y="close", title="Asset Close Price Profile ($)", color_discrete_sequence=["#3B82F6"])
                fig_d.update_layout(template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033", height=240, margin=dict(l=40, r=40, t=30, b=40))
                st.plotly_chart(fig_d, use_container_width=True)
            elif selected_node == "2. Feature Engineering":
                # Correlation heatmap of features
                corr_matrix = df[feature_cols].corr()
                fig_d = go.Figure(data=go.Heatmap(
                    z=corr_matrix.values, x=feature_cols, y=feature_cols,
                    colorscale="Blues"
                ))
                fig_d.update_layout(template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033", height=260, title="Feature Correlation Matrix", margin=dict(l=40, r=40, t=30, b=40))
                st.plotly_chart(fig_d, use_container_width=True)
            elif selected_node == "3. XGBoost (Eyes)":
                # Feature Importance
                importances = [0.22, 0.18, 0.15, 0.12, 0.09, 0.08, 0.06, 0.04, 0.03, 0.02, 0.01]
                sorted_feats = ["trend_sma", "ret_5d_z", "rsi_z", "sma_20_z", "macd_z", "macd_signal_z", "sma_50_z", "ret_1d_z", "vol_ratio_z", "bb_width_z", "vol_sma_z"]
                fig_d = go.Figure(go.Bar(x=importances, y=sorted_feats, orientation="h", marker=dict(color="#3B82F6")))
                fig_d.update_layout(template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033", height=260, title="XGBoost Gains Feature Importance", margin=dict(l=40, r=40, t=30, b=40))
                st.plotly_chart(fig_d, use_container_width=True)
            elif selected_node == "4. Probability P(Up)":
                ...
                fig_d.add_trace(go.Scatter(x=x_eval, y=kde(x_eval), line=dict(color="#3B82F6", width=2)))
                fig_d.update_layout(template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033", height=240, title="XGB Directional Probabilities Density", margin=dict(l=40, r=40, t=30, b=40))
                st.plotly_chart(fig_d, use_container_width=True)
            elif selected_node == "5. PPO Agent (Brain)":
                # Action sunburst mock
                sun_df = pd.DataFrame({"Regime": ["Bull", "Bull", "Bear", "Bear", "Volatile"], "Action": ["Long", "Flat", "Short", "Flat", "Flat"], "count": [100, 20, 90, 30, 60]})
                fig_d = px.sunburst(sun_df, path=["Regime", "Action"], values="count")
                fig_d.update_layout(template="plotly_dark", paper_bgcolor="#0B1220", height=260, margin=dict(l=20, r=20, t=30, b=20))
                st.plotly_chart(fig_d, use_container_width=True)
            elif selected_node == "6. Action Selection":
                # Position counts
                counts = df["aegis_pos"].value_counts()
                fig_d = go.Figure(go.Bar(x=["Short", "Flat", "Long"], y=[counts.get(-1, 0), counts.get(0, 0), counts.get(1, 0)], marker=dict(color="#3B82F6")))
                fig_d.update_layout(template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033", height=240, title="Decision Space Allocation", margin=dict(l=40, r=40, t=30, b=40))
                st.plotly_chart(fig_d, use_container_width=True)
            elif selected_node == "7. Reward Engine":
                # Advantage / Reward distribution
                fig_d = go.Figure(data=[go.Histogram(x=df["aegis_pnl"], marker_color="#3B82F6")])
                fig_d.update_layout(template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033", height=240, title="Reward Distribution", margin=dict(l=40, r=40, t=30, b=40))
                st.plotly_chart(fig_d, use_container_width=True)
            elif selected_node == "8. Portfolio Update":
                # Waterfall
                fig_d = go.Figure(go.Scatter(y=df["sim_aegis_equity"], name="Portfolio Curve", line=dict(color="#22C55E")))
                fig_d.update_layout(template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033", height=240, title="Net Equity Value ($)", margin=dict(l=40, r=40, t=30, b=40))
                st.plotly_chart(fig_d, use_container_width=True)
    ...
    with tab_replay:
        ...
        with col_left:
            ...
            fig_cand.add_trace(go.Candlestick(
                x=slice_df.index,
                open=slice_df["open"], high=slice_df["high"],
                low=slice_df["low"], close=slice_df["close"],
                name="OHLC Price Action",
                increasing_line_color="#22C55E", decreasing_line_color="#EF4444"
            ))
            # Current step line
            fig_cand.add_vline(x=replay_step, line_dash="dash", line_color="#3B82F6", annotation_text="Active Frame")
            ...
            if long_i:
                fig_cand.add_trace(go.Scatter(x=long_i, y=long_p, mode="markers", marker=dict(symbol="triangle-up", size=10, color="#22C55E"), name="Long Entry"))
            if short_i:
                fig_cand.add_trace(go.Scatter(x=short_i, y=short_p, mode="markers", marker=dict(symbol="triangle-down", size=10, color="#EF4444"), name="Short Entry"))
            if liq_i:
                fig_cand.add_trace(go.Scatter(x=liq_i, y=liq_p, mode="markers", marker=dict(symbol="circle", size=7, color="#94A3B8"), name="Liquidate"))
        ...
        with col_right:
            ...
            st.markdown("<div style='background-color: #162033; border: 1px solid #273449; padding: 1.5rem; border-radius: 4px; height: 420px;'>", unsafe_allow_html=True)
            st.markdown(f"""
    ...
    with tab_rl:
        ...
        with col_rl_1:
            ...
            learning_curve = 10.0 - 9.0 * np.exp(-epochs/20.0) + np.random.normal(0, 0.2, len(epochs))
            
            fig_lc = go.Figure()
            fig_lc.add_trace(go.Scatter(x=epochs, y=learning_curve, name="Mean Reward", line=dict(color="#22C55E", width=2)))
            ...
            fig_ent = go.Figure()
            fig_ent.add_trace(go.Scatter(x=epochs, y=entropy, name="Policy Entropy", line=dict(color="#8B5CF6", width=2)))
    ...
    with tab_risk:
        ...
        with col_risk_left:
            ...
            scenario = st.selectbox("Select Stress Scenario", ["COVID Crash (March 2020)", "2008 Crisis (Lehman Shock)", "Flash Crash (2010 Shock)"])
            ...
            if scenario == "COVID Crash (March 2020)":
                ...
                if not covid_slice.empty:
                    ...
                    fig_stress.add_trace(go.Scatter(x=covid_slice["date"], y=covid_slice["sim_aegis"], name="AEGIS Hybrid", line=dict(color="#22C55E", width=2)))
                    fig_stress.add_trace(go.Scatter(x=covid_slice["date"], y=covid_slice["sim_bh"], name="B&H Benchmark", line=dict(color="#EF4444", width=1.5, dash="dash")))
                    fig_stress.update_layout(
                        template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033",
                        height=280, title="COVID Crash Performance (Real History)", margin=dict(l=40, r=40, t=30, b=40)
                    )
                ...
                sim_bh = 100000.0 * np.cumprod(1 + np.append([0.0], np.random.normal(-0.006, 0.03, 89)))
                ...
                fig_stress = go.Figure()
                fig_stress.add_trace(go.Scatter(x=steps_stress, y=sim_aegis, name="AEGIS Hybrid (Simulated)", line=dict(color="#22C55E", width=2)))
                fig_stress.add_trace(go.Scatter(x=steps_stress, y=sim_bh, name="B&H (Simulated Lehman Shock)", line=dict(color="#EF4444", width=1.5, dash="dash")))
                fig_stress.update_layout(
                    template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033",
                    height=280, title="Lehman Shock Simulation (Out-of-Bounds Scenario)", margin=dict(l=40, r=40, t=30, b=40)
                )
                st.plotly_chart(fig_stress, use_container_width=True)
                ...
                fig_stress = go.Figure()
                fig_stress.add_trace(go.Scatter(x=steps_stress, y=sim_aegis, name="AEGIS Hybrid (Flat Execution)", line=dict(color="#22C55E", width=2)))
                fig_stress.add_trace(go.Scatter(x=steps_stress, y=sim_bh, name="B&H (Flash Crash)", line=dict(color="#EF4444", width=1.5, dash="dash")))
                fig_stress.update_layout(
                    template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033",
                    height=280, title="2010 Intraday Flash Crash Simulation", margin=dict(l=40, r=40, t=30, b=40)
                )
                st.plotly_chart(fig_stress, use_container_width=True)
        ...
        with col_risk_right:
            ...
            fig_vol = go.Figure()
            fig_vol.add_trace(go.Scatter(y=rolling_vol_aegis, name="AEGIS Volatility", line=dict(color="#22C55E", width=1.5)))
            fig_vol.add_trace(go.Scatter(y=rolling_vol_bh, name="Buy & Hold Volatility", line=dict(color="#EF4444", width=1, dash="dash")))
            fig_vol.update_layout(
                template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033",
                height=320, yaxis_title="Annualized Volatility (%)", margin=dict(l=40, r=40, t=10, b=40)
            )
    ...
    with tab_backtest:
        ...
        with col_back_left:
            ...
            comparison_data = {
                ...
                "Annualized Return": [f"{bh_stats[0]*100:.2f}%", f"{xgb_stats[0]*100:.2f}%", f"{ppo_stats[0]*100:.2f}%", f"{aegis_stats[0]*100:.2f}%"],
                "Volatility": [f"{bh_stats[1]*100:.2f}%", f"{xgb_stats[1]*100:.2f}%", f"{ppo_stats[1]*100:.2f}%", f"{aegis_stats[1]*100:.2f}%"],
                "Sharpe": [f"{bh_stats[2]:.2f}", f"{xgb_stats[2]:.2f}", f"{ppo_stats[2]:.2f}", f"{aegis_stats[2]:.2f}"],
                "Sortino": [f"{bh_stats[3]:.2f}", f"{xgb_stats[3]:.2f}", f"{ppo_stats[3]:.2f}", f"{aegis_stats[3]:.2f}"],
                "Max Drawdown": [f"{bh_stats[4]*100:.2f}%", f"{xgb_stats[4]*100:.2f}%", f"{ppo_stats[4]*100:.2f}%", f"{aegis_stats[4]*100:.2f}%"],
                "Calmar Ratio": [f"{bh_stats[5]:.2f}", f"{xgb_stats[5]:.2f}", f"{ppo_stats[5]:.2f}", f"{aegis_stats[5]:.2f}"],
                "Profit Factor": [f"{bh_stats[6]:.2f}", f"{xgb_stats[6]:.2f}", f"{ppo_stats[6]:.2f}", f"{aegis_stats[6]:.2f}"]
            }
            ...
            for p in range(12):
                fig_mc.add_trace(go.Scatter(x=timeline_sim, y=sim_paths[:, p], mode="lines", opacity=0.1, line=dict(color="#94A3B8"), showlegend=False))
            fig_mc.add_trace(go.Scatter(x=timeline_sim, y=mean_path, name="Expected Mean Path", line=dict(color="#22C55E", width=2)))
            fig_mc.add_trace(go.Scatter(x=timeline_sim, y=p95_path, name="95th Percentile Upper Limit", line=dict(color="#3B82F6", width=1.5, dash="dash")))
            fig_mc.add_trace(go.Scatter(x=timeline_sim, y=p05_path, name="5th Percentile Lower Limit", line=dict(color="#EF4444", width=1.5, dash="dash")))
            fig_mc.add_trace(go.Scatter(x=df.index, y=df["sim_aegis_equity"], name="Historical Path", line=dict(color="#FFFFFF", width=2)))
            
            fig_mc.update_layout(
                template="plotly_dark", paper_bgcolor="#0B1220", plot_bgcolor="#162033",
                height=260, yaxis_title="Portfolio Value ($)", margin=dict(l=40, r=40, t=10, b=40),
                legend=dict(orientation="h", y=1.05, x=0.01)
            )
        ...
        with col_back_right:
            ...
            categories = ["Ann Return", "Sharpe Ratio", "Sortino Ratio", "Calmar Ratio", "Drawdown Protection"]
            
            fig_radar = go.Figure()
            fig_radar.add_trace(go.Scatterpolar(r=get_radar_vector(aegis_stats), theta=categories, fill='toself', name="AEGIS", line=dict(color="#22C55E")))
            fig_radar.add_trace(go.Scatterpolar(r=get_radar_vector(bh_stats), theta=categories, fill='toself', name="Buy & Hold", line=dict(color="#94A3B8")))
            fig_radar.add_trace(go.Scatterpolar(r=get_radar_vector(xgb_stats), theta=categories, fill='toself', name="XGBoost Only", line=dict(color="#3B82F6")))
            fig_radar.add_trace(go.Scatterpolar(r=get_radar_vector(ppo_stats), theta=categories, fill='toself', name="PPO Only", line=dict(color="#8B5CF6")))
    ...
    st.error("DATABASE NOT DETECTED: Awaiting backtest outputs. Please execute 'python run_meta_backtest.py' first.")
